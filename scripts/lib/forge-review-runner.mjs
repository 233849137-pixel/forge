...
export function parseReviewRunnerArgs(argv = []) {
  const args = {
    projectId: "",
    workspace: "",
    taskpackId: "",
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === "--artifact") {
      args.artifact = requireArg(value, "artifact");
      index += 1;
      continue;
    }
    ...
    if (token === "--review-command") {
      args.reviewCommand = requireArg(value, "review-command");
      index += 1;
    }
  }
...
export async function runReviewVerification(options, runtime = {}) {
  const workspace = requireArg(options.workspace, "workspace");
  const projectId = requireArg(options.projectId, "project-id");
  const artifact = requireArg(options.artifact || "patch", "artifact");
  ...
  const externalCapability =
    runtime.detectExternalExecutionCapability?.("reviewer") ||
    detectExternalExecutionCapability("reviewer", runtime.env || process.env, {
  ...
  if (externalCapability) {
    ...
    if (options.executeIfReady) {
      ...
      return {
        ...
        summary: `${executionSummary}，已进入 ${projectId} 的 ${artifact} 规则审查。`,
        checks: [
          ...
          ...(componentIds.length > 0
            ...
            : []),
          { name: "artifact", status: "pass", summary: `已接收待审工件：${artifact}` },
          {
    ...
    return {
      ...
      summary: `已检测到外部审查执行器 ${providerSummary}，可继续执行 ${projectId} 的 ${artifact} 规则审查（TaskPack：${taskPackRef}）。`,
      checks: [
        { name: "workspace", status: "pass", summary: `已找到工作区：${workspace}` },
        { name: "taskpack", status: "pass", summary: `已选择 TaskPack：${taskPackRef}` },
        ...(componentIds.length > 0
  ...
  const executableInfo =
    runtime.detectExecutableInfo?.("git") ||
    (() => {
      const path = resolveExecutable("git");
      return path ? { path, version: null } : null;
    })();
  const gitBinary = executableInfo?.path || null;
  const gitVersion = executableInfo?.version || null;

  if (gitBinary) {
    if (options.executeIfReady) {
      const executionCommand = options.reviewCommand
        ? splitCommandString(options.reviewCommand)
        : [gitBinary, "status", "--short"];
      const execution = await executeCommand(executionCommand, workspace, runtime);
      const executionSummary = execution.ok
      ...
      return {
        ...
        checks: [
          ...
          { name: "artifact", status: "pass", summary: `已接收待审工件：${artifact}` },
          {
            name: "git",
            status: "pass",
            summary: `已检测到 ${gitBinary}${gitVersion ? ` · ${gitVersion}` : ""}`
          },
          {
            name: "execution",
            status: execution.ok ? "pass" : "fail",
    ...
    return {
      ...
      evidenceLabel: evidence.evidenceLabel,
      executedCommand: evidence.executedCommand,
      summary: `已检测到 git (${gitBinary}${gitVersion ? `, ${gitVersion}` : ""})，可继续执行 ${projectId} 的 ${artifact} 规则审查（TaskPack：${taskPackRef}）。`,
      checks: [
        { name: "workspace", status: "pass", summary: `已找到工作区：${workspace}` },
        { name: "taskpack", status: "pass", summary: `已选择 TaskPack：${taskPackRef}` },
        ...(componentIds.length > 0
  ...
  return {
    ...
    evidenceLabel: evidence.evidenceLabel,
    executedCommand: evidence.executedCommand,
    summary: `已完成 ${projectId} 的 ${artifact} 规则审查（TaskPack：${taskPackRef}），当前使用本地 Reviewer 合同模式。`,
    checks: [
      { name: "workspace", status: "pass", summary: `已找到工作区：${workspace}` },
      { name: "taskpack", status: "pass", summary: `已选择 TaskPack：${taskPackRef}` },
      ...(componentIds.length > 0
        ...
        : []),
      { name: "artifact", status: "pass", summary: `已接收待审工件：${artifact}` },
      { name: "review-policy", status: "pass", summary: "基础规则审查策略已加载" },
      evidence.evidenceCheck
    ]
  };
}
...
export async function main(argv = process.argv.slice(2)) {
  const args = parseReviewRunnerArgs(argv);
  const result = await runReviewVerification(args);
