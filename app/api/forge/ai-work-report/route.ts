...
async function executeShell(commandPreview: string, cwd?: string | null) {
  ...
  if (!binary) {
    throw new ForgeApiError("当前工作汇报缺少可执行命令", "FORGE_BACKEND_NOT_READY", 409);
  }
...
function extractReport(parsed: Record<string, unknown> | null, fallback: string) {
...
export async function POST(request: Request) {
  try {
    ...
    const scope = readOptionalString(body, "scope", "汇报范围");
    const projectId = scope === "portfolio" ? null : readOptionalString(body, "projectId", "项目 ID");
    const triggeredBy = readOptionalString(body, "triggeredBy", "触发人") ?? "Forge Home · AI工作汇报";
    const snapshot = loadDashboardSnapshot();
    const project = projectId ? snapshot.projects.find((item) => item.id === projectId) : null;
    const isPortfolioScope = scope === "portfolio";

    if (!isPortfolioScope && !project) {
    ...
    if (!commandTemplate) {
      throw new ForgeApiError(
        "当前还没有配置可用的 CEO 总控工作汇报后端。",
        "FORGE_BACKEND_NOT_READY",
        409
      );
    }

    const resolvedProjectId = isPortfolioScope ? "portfolio" : project!.id;
    const resolvedProjectName = isPortfolioScope ? "全部项目" : project!.name;
    const stage = isPortfolioScope
      ? "组合视图"
      : snapshot.workflowStates.find((item) => item.projectId === resolvedProjectId)?.currentStage ?? "项目接入";
    const workspacePath = isPortfolioScope
      ? process.cwd()
      : snapshot.projectProfiles.find((item) => item.projectId === resolvedProjectId)?.workspacePath ?? process.cwd();
    ...
    if (isPortfolioScope) {
      commandPreview = appendMessageSuffix(
        commandPreview,
        `请基于以下全部项目组合信息生成首页工作汇报，按组合视角总结重点，不要只盯单个项目。\n\n${portfolioSummary}`
      );
    }
    ...
    const report = extractReport(parsed, stdoutText || stderrText);

    if (!result.ok) {
      throw new ForgeApiError(
        report || `工作汇报生成失败，退出码 ${result.code ?? "unknown"}`,
    ...
    return forgeSuccess({
      ...
      provider: process.env.FORGE_PM_EXEC_PROVIDER?.trim() ?? "OpenClaw PM",
      commandPreview,
      summary: `已通过 ${process.env.FORGE_PM_EXEC_PROVIDER?.trim() ?? "OpenClaw PM"} 生成项目工作汇报。`,
      report
    });
  } catch (error) {
